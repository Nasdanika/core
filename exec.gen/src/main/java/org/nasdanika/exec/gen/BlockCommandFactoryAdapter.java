...
public class BlockCommandFactoryAdapter extends BlockExecutionParticipantAdapter implements CommandFactory {
	...
	public Command create(Context context) {
		...
		List<CommandFactory> tryBlock = target.getTry().stream().map(e -> Objects.requireNonNull(EObjectAdaptable.adaptTo(e, CommandFactory.class), "Cannot adapt to CommandFactory: " + e)).toList();
		Command tryCommand = Util.asCommandFactory(tryBlock).create(context);
		
		Command catchCommand = null; 		
		if (!target.getCatch().isEmpty()) {
			List<CommandFactory> catchBlock = target.getCatch().stream().map(e -> Objects.requireNonNull(EObjectAdaptable.adaptTo(e, CommandFactory.class), "Cannot adapt to CommandFactory: " + e)).toList();
		...
		if (!target.getFinally().isEmpty()) {
			List<CommandFactory> finallyBlock = target.getFinally().stream().map(e -> Objects.requireNonNull(EObjectAdaptable.adaptTo(e, CommandFactory.class), "Cannot adapt to CommandFactory: " + e)).toList();
